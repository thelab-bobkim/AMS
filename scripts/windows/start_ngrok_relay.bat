...
:: ngrok 설치 경로 (WinGet 설치 경로)
SET NGROK_PATH=C:\Users\hp\AppData\Local\Microsoft\WinGet\Packages\Ngrok.Ngrok_Microsoft.Winget.Source_8wekyb3d8bbwe\ngrok.exe
...
:: ── ngrok 실행 로그 기록 ─────────────────────────────────────
echo === ngrok 시작 === >> "%LOG_FILE%"
echo %date% %time% >> "%LOG_FILE%"
echo Domain: %NGROK_DOMAIN% >> "%LOG_FILE%"
echo Target: %SENSELINK_URL% >> "%LOG_FILE%"
